...
async function initializeDatabase() {
    ...
    try {
        ...
        if (admins.length === 0) {
            const hashedPassword = await bcrypt.hash('Jacob12!', 10);
            await connection.query(
        ...
        if (venues[0].count === 0) {
            for (let i = 101; i <= 201; i++) {
                await connection.query(
                    ...
                    [`Room ${i}`, 'A', 50, `Lecture room ${i} in Block A`]
                );
            }
            for (let i = 10; i <= 101; i++) {
                await connection.query(
                    'INSERT INTO venues (name, block, capacity, description) VALUES (?, ?, ?, ?)',
                    [`Room ${i}`, 'B', 40, `Lecture room ${i} in Block B`]
                );
            }
            for (let i = 30; i <= 50; i++) {
                await connection.query(
                    'INSERT INTO venues (name, block, capacity, description) VALUES (?, ?, ?, ?)',
                    [`Room ${i}`, 'C', 30, `Lecture room ${i} in Block C`]
                );
            }
            for (let i = 1; i <= 5; i++) {
                await connection.query(
                    'INSERT INTO venues (name, block, capacity, description) VALUES (?, ?, ?, ?)',
                    [`Library Room ${i}`, 'Library', 20, `Library room ${i}`]
                );
            }
            for (let i = 1; i <= 20; i++) {
                await connection.query(
                    'INSERT INTO venues (name, block, capacity, description) VALUES (?, ?, ?, ?)',
                    [`Lab ${i}`, 'ComputerLab', 30, `Computer laboratory ${i}`]
                );
            }
